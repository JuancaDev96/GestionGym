...
namespace GestionGym.Repositorios.Implementaciones
{
    public class MaestroRepository : BaseRepository<Maestro>, IMaestroRepository
    {
        ...
        public async Task<(List<MaestroDetalleResponse> collection, int total)> ObtenerDetalleMaestroByIdMaestro(int idMaestro, int pagina, int filas)
        {
            var coleccion = await _contexto.Maestrodetalles
                                 .Where(p => p.Idmaestro == idMaestro && p.Estado)
                                 .AsNoTracking()
                                 .OrderBy(p => p.Id)
                                 .Skip((pagina - 1) * filas)
                                 .Take(filas)
